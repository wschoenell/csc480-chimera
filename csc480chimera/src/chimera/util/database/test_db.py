...
class database :
    ...
    def Insert_Exposure(self,path,G_RA,G_DEC):
         try:
            self.cursor.execute("SELECT Insert_Exposure(%s,%s,%s)",(path,G_RA,G_DEC))
            r = self.cursor.fetchall()
            print "row count %s" % self.cursor.rowcount
    ...
    def Insert_INTO_Star(self,_RA,_DEC,flux,seeing,magnitude,exposureid):
         try:
    ...
    def WiteEC_INTO_Extiction(self,data):
        try:
           if len(data) == 2 : 
              EC = data[0]
              print "EC %s" % EC 
              ...
              while i<len(starid):
                 STARID = starid[i]
                 self.cursor.execute("SELECT Insert_Star_Extinction(%s,%s)",(ID,STARID))
                 i = i+1
    ...
    def get_ExposuresForExtinction(self,_ra,_dec):
        try:
...
test = database()
test.get_Flux("201014")
test.Insert_Exposure("/usr/data/file1","123.34","125.55")
test.get_Magnitude("201015")
